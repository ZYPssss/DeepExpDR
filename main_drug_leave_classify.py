...
from train_val_classify import train_val_drug_leave_1, class_train, train_val_drug_leave
...
use_sc = False
single_cells = 10
args1.device = torch.device("cuda:" + str(2)) if torch.cuda.is_available() else torch.device("cpu")
device = args1.device
batch_size = 256
...
cline_glofeat = np.load('./data/Binary_class/cline_glofeat_drug_cold.npy')
drug_glofeat = np.load('./data/Binary_class/drug_glofeat_drug_cold.npy')
...
#model = torch.load('./result/best_drug_leave_out_model_classify/best_drug_leave_out_model.pth').to(device)
criterion = nn.MSELoss(reduction="none")
opt = torch.optim.Adam(model.parameters(), lr=0.001)

print('-------------------------------train test val---------------------------------------')
train_val_drug_leave(model, class_model, featuring_train, featuring_validation_drug, use_sc, device, 200, criterion, opt, drug_data, drug_data1, scf_tr,
                     drug_scf_idx)
